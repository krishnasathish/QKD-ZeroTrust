...
B) Files needed for noise model: B.1) All files in A)

B.2) qkd_noise_model.py (contains main())

C) Eavesdropping C.1) All files in A)
...
Brief summary:
    ...
    The basic experiment is noiseless, but existing base classes can be easily overridden (OOP) to implement custom noise models/eavesdropping.
    The functionality to validate the key by comparing the first half of the key for bit errors has been implemented.
    Measurement outcomes are implemented using numpy's random number generation.
    Detailed transcripts print a summary of the results of the experiment.
...
--> the experiment is divided into modular phases to build the objects, running the experiment for transmitting qubits from Alice to Bob, key generation phase based on bases used by Alice and Bob; method to find the error rate in the generated key.
